function currentLang() {
  try {
    var l = (document.documentElement && document.documentElement.lang) || 'en';
    return (l.toLowerCase().indexOf('zh') === 0) ? 'zh-CN' : (l || 'en');
  } catch(e) { return 'en'; }
}
function favLabel(active) {
  var lang = currentLang();
  if (lang === 'zh-CN') return active ? '取消收藏' : '收藏';
  if (lang === 'es') return active ? 'Quitar de favoritos' : 'Favorito';
  if (lang === 'fr') return active ? 'Retirer des favoris' : 'Favori';
  return active ? 'Unfavorite' : 'Favorite';
}
function initFavoritesLabelsFallback(){
  try {
    document.querySelectorAll('.favorite-button').forEach(function(btn){
      var ft = btn.querySelector('.favorite-text'); if (!ft) return;
      var isActive = btn.classList.contains('active');
      ft.textContent = favLabel(isActive);
      btn.setAttribute('aria-label', favLabel(isActive));
    });
  } catch(e) {}
}
/**
...
function initFavorites() {
  ...
  favoriteButtons.forEach(button => {
    ...
    if (gameId && favorites[gameId]) {
      ...
      button.setAttribute('aria-label', favLabel(false));
      (function(){var __ft=button.querySelector('.favorite-text'); if(__ft) __ft.textContent = favLabel(false);})();
    }
...
function initGamePlayTracking() {
  ...
  if (gameElement && gameId) {
    ...
    function recordGamePlay() {
      ...
      try {
        var titleEl = document.querySelector('h1, .game-title');
        var t = (titleEl && titleEl.textContent) || (document.querySelector('meta[property=\"og:title\"]')?.getAttribute('content')) || gameId;
        var list = [];
        try { list = JSON.parse(localStorage.getItem('play_history')||'[]'); } catch {}
        list.push({ game_id: gameId, game_title: t, played_at: new Date().toISOString() });
        localStorage.setItem('play_history', JSON.stringify(list.slice(-500)));
        try { if (window.AppAuth && window.AppAuth.syncUp) window.AppAuth.syncUp(); } catch {}
      } catch (e) {}
    }
...
function showNotification(message, type = 'default') {
  ...
  setTimeout(() => {
    ...
    setTimeout(() => { try { notification.remove(); } catch(e){} }, 300);
  }, 3000);
}
